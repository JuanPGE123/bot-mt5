...
input long    InpMagicNumber      = 20251107;    // Magic Number (distinto del EA base para no mezclar posiciones si corren juntos en el mismo simbolo)
...
input double  InpADX_MinLevel     = 14.0;  // ADX minimo obligatorio [opt 10-30 step 2]

input string  Inp_Section4b       = "===== SMC (BLOQUEANTE) =====";
input bool    InpUseSMCFilter     = true;  // Requerir confluencia con OB o FVG
input int     InpSMC_Lookback     = 20;    // Velas hacia atras para buscar FVG/OB [opt 10-40 step 5]
input double  InpSMC_ZoneBufferPts= 5.0;   // Tolerancia en puntos p/ considerar "dentro" de zona [opt 0-20 step 5]
