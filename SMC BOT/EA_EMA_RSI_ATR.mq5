...
int GetSignal(double &lotFactor)
  {
   lotFactor = 1.0;
   double emaFast[3], emaSlow[3], emaTrend[2], rsi[2], atr[2];
   ...
   if(InpUseADXFilter)
     {
      double adxMain[2];
      if(CopyBuffer(handleADX, 0, 0, 2, adxMain) == 2 && adxMain[1] < InpADX_MinLevel)
         Log("DEBUG ADX bajo (" + DoubleToString(adxMain[1],1) + "), no bloquea entrada.");
     }
   ...
   if(InpUseTrendFilter && CopyBuffer(handleEMA_Trend, 0, 0, 2, emaTrend) == 2)
     {
      trendPrice = iClose(_Symbol, InpTF, 1);
      trendReady = true;
     }
